...
class MHCA_stage(nn.Module):
    ...
    def forward(self, inputs):
        """foward function"""
        att_outputs = [self.InvRes(inputs[0])]  #conv_features
        for x, encoder in zip(inputs, self.mhca_blks):
            # [B, C, H, W] -> [B, N, C]
            _, _, H, W = x.shape
            x = x.flatten(2).transpose(1, 2)
            
            #print("MHCA before encoder", x.shape)
            
            ot= encoder(x, size=(H, W))
            
            #print("MHCA after encoder", ot.shape)
            
            att_outputs.append(ot)
            
            
        out_concat = torch.cat(att_outputs, dim=1)
        out = self.aggregate(out_concat)
        
        #print("cat_att",out.shape)

        return out


# ChangeFormer's Encoder block
class CDEncoder(nn.Module) :
    def __init__(
        ...
        from models.ChangeFormer import Block
        super().__init__()
        
        self.encoder_blks =  nn.ModuleList([Block(
            dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=drop_path_list[i], norm_layer=norm_layer,
            sr_ratio=sr_ratio) for i in range(num_layers)])
        self.norm = norm_layer(embed_dim)


    def forward(self, x, size):
        B = x.shape[0] #[B,4096,64] 4096=256*256
        # stage
        H1, W1 = size 
        for i, blk in enumerate(self.encoder_blks):
            x = blk(x, H1, W1)  
        x = self.norm(x)#[B,4096,64]
        x = x.reshape(B, H1, W1, -1).permute(0, 3, 1, 2).contiguous()#[B,64,64,64]
        
        return x
        

# Apply ChangeFormer's Encoder to every stage
class MHCA_stage_CD(nn.Module):
    """Multi-Head Convolutional self-Attention stage comprised of `MHCAEncoder`
    layers."""
    def __init__(
        self,
        ...
        #transencoder
        self.CDtrans_blks = nn.ModuleList([
            CDEncoder(
                embed_dim=embed_dim, 
                num_heads=num_heads, 
                mlp_ratio=mlp_ratio, 
    ...
    def forward(self, inputs):
        """foward function"""
        att_outputs = [self.InvRes(inputs[0])]  #conv_features
        for x, encoder in zip(inputs, self.CDtrans_blks):
            # [B, C, H, W] -> [B, N, C]
            _, _, H, W = x.shape
            x = x.flatten(2).transpose(1, 2)
            
            #print("CDE before encoder", x.shape)
            
            ot = encoder(x, size=(H, W))
            
            #print("CDE after encoder", ot.shape)
...
class MPViT(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super().__init__()
        print("The number of paths in each layer:", num_path)
        self.num_classes = num_classes
        self.num_stages = num_stages
        self.CDEncoder = CDEncoder
        ...
        if CDEncoder == False :
            ...
            print("Use ChangeFormer's transformer block")
            self.encoder_stages = nn.ModuleList([
                MHCA_stage_CD (embed_dim = embed_dims[idx],
    ...
    def forward_features(self, x):
        ...
        outs = []
        # x's shape : [B, C, H, W]#[2,3,256,256]
        # print('x:',x.shape)
        x = self.stem(x)  # Shape : [B, C, H/4, W/4]#[2,64,64,64]
        # print('conv stem',x.shape)
        for idx in range(self.num_stages):
            att_inputs = self.patch_embed_stages[idx](x)
            # if idx == 0:
            #     print('att:',att_inputs[0].shape,att_inputs[1].shape)
            # else:
            #     print('att:',att_inputs[0].shape,att_inputs[1].shape,att_inputs[2].shape)
            x = self.encoder_stages[idx](att_inputs)
            # print('x',idx,x.shape)
            outs.append(x)
        return outs
...
def mpvit_base_old(**kwargs):
    ...
    model = MPViT(
        ...
        num_path=[2, 3, 3, 3],
        num_layers=[1, 3, 8, 3],
        embed_dims=[128, 224, 368, 480, 480],
        mlp_ratios=[4, 4, 4, 4],
        num_heads=[8, 8, 8, 8],
        **kwargs,
    )
...
@register_model
def mpvit_mixCD( path=[2,3,3,3], **kwargs):
    model = MPViT(
        CDEncoder=True,
        img_size=256,
        num_stages=4,
